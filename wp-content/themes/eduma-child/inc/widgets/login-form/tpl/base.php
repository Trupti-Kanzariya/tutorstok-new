	...
	<div class="thim-login-container">
		<div class="thim-login form-submission-login">
			...
			function handle_custom_login() {
				...
				if (empty($_POST['log'])) {
					$field_errors['log'] = 'Please enter username';
				}
			...
			<?php if (!empty($_SESSION['login_error'])) : ?>
				<div class="login-error-message" style="color: red; margin-bottom: 15px;">
					...
					if (strpos($error, 'not registered') !== false) {
						echo 'Please first register and try again later.';
					} elseif (strpos($error, 'incorrect password') !== false) {
			...
			<form name="loginform" id="loginform" method="post" action="" >
				...
				<p class="login-username">
					...
					placeholder="Email Address" value="<?php echo isset($_POST['log']) ? esc_attr($_POST['log']) : ''; ?>">
...
<script>
// Client-side validation to show errors immediately
	document.getElementById('loginform').addEventListener('submit', function(e) {
		let isValid = true;

    // Validate username
		const username = document.getElementById('user_login');
		const usernameError = document.getElementById('log-error');
