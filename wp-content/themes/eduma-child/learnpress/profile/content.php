...
<article id="profile-content" class="lp-profile-content">
	<div id="profile-content-<?php echo esc_attr( $tab_key ); ?>">
		<?php
			...
			$profile_url = learn_press_user_profile_link(); // current user's profile base URL
			$settings_url = trailingslashit($profile_url) . 'settings';
		?>
		<div class="lp-profile-header" style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px;">
			...
			<div class="custom-lp-profile-content">
				<a href="<?php echo esc_url($settings_url); ?>" class="custom-lp-profile-link">
					...
					if ( in_array( 'lp_teacher', $user_roles ) ) {
						$profile_message = 'Empowering students with knowledge!!!';
					} else {
						$profile_message = 'Making great progress!!';
					}
					?>
					<div class="custom-lp-profile-description"><?php echo esc_html( $profile_message ); ?></div>
