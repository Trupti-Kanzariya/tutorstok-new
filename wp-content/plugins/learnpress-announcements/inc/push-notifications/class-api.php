...
class Api {
	...
	public function push_notifications( $user_ids, $notification_id, $request ) {
		...
		foreach( $device_tokens as $device_token ) {
			$this->request_send_push_notification( $device_token, $notification, $request );
		}

		// !important: use https://fcm.googleapis.com/batch deprecated on June 21, 2024
		// $this->request_send_push_notifications( $database, $device_tokens, $notification, $request );

		return true;
	}

	private function request_send_push_notification( $device_token, $notification, $request ) {
		...
		if ( ! empty( $response->failure ) ) {
			// TODO: delete device token or not?
		}
