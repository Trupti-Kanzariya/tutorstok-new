...
public function html_count_courses($instructor): string {
    ...
    try {
        ...
        $course_count = $instructor_statistic['published_course'];
        $course_text = _n('Course', 'Courses', $course_count, 'learnpress');

        $sections = [
            'wrapper'     => '<div class="instructor-course-count">',
            'content'     => sprintf(
                '<div class="course-number">%d</div><div class="course-label">%s</div>',
                $course_count,
                $course_text
            ),
            'wrapper_end' => '</div>',
	...
	public function html_count_students($instructor): string {
		...
		try {
			...
			$instructor_statistic = $instructor->get_instructor_statistic();
			$student_count = $instructor_statistic['total_student'];
			$student_text = _n('Student', 'Students', $student_count, 'learnpress');

			$sections = [
				'wrapper'     => '<div class="instructor-student-count">',
				'content'     => sprintf(
					'<div class="student-number">%d</div><div class="student-label">%s</div>',
					$student_count,
					$student_text
				),
				'wrapper_end' => '</div>',
