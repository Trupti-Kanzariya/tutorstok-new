...
if ( ! class_exists( 'LP_Addon_Assignment' ) ) {
	...
	class LP_Addon_Assignment extends LP_Addon {
		...
		public function assignment_enqueue_scripts() {
			if ( function_exists( 'learn_press_is_course' ) && learn_press_is_course() ) {
				...
				wp_enqueue_style(
					...
					LP_Addon_Assignment_Preload::$addon->get_plugin_url( "assets/dist/css/assignment{$is_rtl}{$min}.css" ),
					[],
					$ver
				);
				...
				if ( learnpress_is_assignment_page() ) {
					...
					wp_localize_script( 'lp-assignment', 'lpAssignment', learn_press_assignment_single_args() );
					//$scripts = learn_press_assets();
					//$scripts->add_script_data( 'lp-assignment', learn_press_assignment_single_args() );
				}
			}
		}
		...
		/*public static function start_assignment() {
			...
			try {
				...
				if ( $json['data']['status'] == 400 ) {
					...
					wp_redirect( LP_Helper::getUrlCurrent() );
					exit();
				}
			} catch ( \Throwable $e ) {
				$message = [
		...
		protected function _includes() {
			include_once LP_ADDON_ASSIGNMENT_INC_PATH . 'Filters/class-lp-assignment-filter.php';
		...
		public static function user_start_assignment( UserModel $user, CourseModel $course, AssignmentPostModel $assignment ) {
			try {
				...
				$userAssignmentModel->status     = LP_ITEM_STARTED;
