	...
	class LP_Jwt_Assignment_V1_Controller extends LP_REST_Jwt_Posts_Controller {
		...
		public function delete_submit_file( $request ) {
			...
			try {
				...
				if ( ! $courseModel ) {
					throw new Exception( esc_html__( 'Course not found', 'learnpress-assignment' ) );
				}

				$assignmentModel = AssignmentPostModel::find( $assignment_id, true );
				if ( ! $assignmentModel ) {
					throw new Exception( esc_html__( 'Assignment not found', 'learnpress-assignment' ) );
				}

				if ( ! $user ) {
					throw new Exception( esc_html__( 'User is not available!', 'learnpress-assignment' ) );
				}
				...
				if ( empty( $file ) ) {
					throw new Exception( esc_html__( 'File is not available!', 'learnpress-assignment' ) );
				}
